...
import { motion, useSpring, useMotionValue, useAnimationControls, AnimatePresence } from 'motion/react';
...
export default function App() {
  ...
  useEffect(() => {
    fetch('/api/wishes')
      .then((r) => r.json())
      .then((rows) => setWishes(shuffle(rows).slice(0, isMobile ? 5 : 10).map(addScatter)))
      .catch(console.error);
  }, []);
  ...
  const handleAddWish = async (wish) => {
    try {
      ...
      setWishes((prev) => [newWish, ...prev].slice(0, isMobile ? 5 : 10));
    ...
    <>
      ...
      <div className="flex flex-col gap-5 relative z-10">
        <motion.img
          ...
          initial={isMobile ? false : { opacity: 0, filter: 'blur(10px)' }}
          animate={{ opacity: 1, filter: 'blur(0px)' }}
          transition={{ duration: 0.6, delay: 0.1 }}
        />
        <motion.h1
          style={{ fontFamily: '"Libre Baskerville", Georgia, serif', fontSize: isMobile ? '28px' : '36px', color: '#5C4A2E', lineHeight: 1.2 }}
          initial={isMobile ? false : { opacity: 0, filter: 'blur(10px)' }}
          animate={{ opacity: 1, filter: 'blur(0px)' }}
  ...
  return (
    <>
    <div className="grain" style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '420px 1fr', width: '100vw', height: '100dvh', overflow: 'hidden' }}>
      ...
      {/* Left column — description (desktop: inline, mobile: slide-in overlay) */}
      {!isMobile && (
      ...
      <div style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        {isMobile && (
          <button
            ...
            className="absolute top-4 left-4 z-50 w-10 h-10 flex items-center justify-center rounded-full bg-white/80 backdrop-blur-sm shadow-md cursor-pointer"
            style={{ color: '#5C4A2E' }}
          >
        ...
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[5] text-stone-500 text-xs font-mono uppercase tracking-[0.25em] pointer-events-none">
          {isMobile ? 'Tap & drag cards' : 'Drag cards around'}
        </div>

        {/* Draggable wishes layer */}
        <div className="absolute z-10" style={{ inset: isMobile ? 16 : 60 }}>
          <DragElements
            dragMomentum={true}
    ...
    {/* Mobile sidebar overlay — outside grid to overlay everything */}
    <AnimatePresence>
      {isMobile && sidebarOpen && (
        ...
        >
          ...
          />
          <motion.div
            className="absolute top-0 left-0 bottom-0 overflow-y-auto flex flex-col justify-between"
            style={{ width: '85vw', maxWidth: '380px', padding: '24px 20px', position: 'relative' }}
            initial={{ x: '-100%' }}
            animate={{ x: 0 }}
            exit={{ x: '-100%' }}
